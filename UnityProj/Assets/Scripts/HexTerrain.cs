...
public class HexTerrain : MonoBehaviour
{
    ...
    unsafe public void Generate(void* pBlock)
    {
        ...
        int* cellTypeCounts = cellTypes + WorldBlock.sz * WorldBlock.sz;
        int nonEmptyCellsCount = *(cellTypeCounts + WorldBlock.terrainTypesCount);
        ...
        for (int x = 0; x < WorldBlock.sz; x++)
        {
            ...
            {
                int ctype = (int)cellTypes[x * WorldBlock.sz + y];
                if (ctype == 0) continue;
    ...
    public void Generate(WorldBlock wb)
    {
